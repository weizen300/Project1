...
def load_data(**kwargs):
    ...
    train_data = torch.tensor(train_data, dtype=torch.float32) / 255.

    # Load the test data
    test_data_input = np.load("test_data.npz")["data"]

    # Make the test data a tensor
    test_data_input = torch.tensor(test_data_input, dtype=torch.float32) / 255.
...
def testing(model, test_data_input):
    ...
    # Save the output
    test_data_output = test_data_output.numpy() * 255.
